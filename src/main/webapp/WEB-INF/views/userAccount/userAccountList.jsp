...
<div class="container">
	<p align="right">
		<a href="userAccountCreate" class="btn btn-info btn-lg"><spring:message
				code="add" /></a>
	</p>
</div>
		...
		<tbody>
			<c:forEach items="${userAccounts}" var="userAccount">
				<tr>
					...
					<td><a
						href="userAccountUpdate?id=<c:out value="${userAccount.id}" />"><spring:message
								code="edit" /></a>,&nbsp; <a
						href="userAccountDelete?id=<c:out value="${userAccount.id}" />"><spring:message
								code="delete" /></a></td>
				</tr>
			</c:forEach>
		</tbody>
